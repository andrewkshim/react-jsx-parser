	...
	#parseExpression = (expression: AcornJSX.Expression, scope?: Scope): any => {
		...
		case 'CallExpression':
			...
			return parsedCallee(...expression.arguments.map(
				arg => this.#parseExpression(arg, { ...scope, ...expression.callee }),
			))
		...
		case 'LogicalExpression':
			...
			const isShortCircuitOperator = expression.operator === '||' || expression.operator === '??'
			if (isShortCircuitOperator && left) return left
			if (
				(expression.operator === '&&' && left)
					|| (expression.operator === '??' && left == null)
					|| (expression.operator === '||' && !left)
			) {
